...
struct CollisionBitMask {
    ...
    static let blackCategory:UInt32 = 0x1 << 4
}

extension GameScene {
    
//Basic stuff to set up the collision bitmasks and the bird, frame of the bird, size of the bird
    func createBird() -> SKSpriteNode {
//Sets the size of the object
        let bird = SKSpriteNode(texture: SKTextureAtlas(named:"player").textureNamed("bird1"))
        bird.size = CGSize(width: 50, height: 50)
        bird.position = CGPoint(x:100, y:0)
...
//Category bit masks
        ...
        bird.physicsBody?.collisionBitMask = CollisionBitMask.pillarCategory | CollisionBitMask.groundCategory | CollisionBitMask.blackCategory
        bird.physicsBody?.contactTestBitMask = CollisionBitMask.pillarCategory | CollisionBitMask.flowerCategory | CollisionBitMask.groundCategory | CollisionBitMask.blackCategory
...
//Added code
    ...
    func createBlack(xPos: Int, yPos: Int, xWidth: Int) -> SKSpriteNode{
        let black = SKSpriteNode(imageNamed: "000000-1")
        black.size = CGSize(width: 5, height: 5)
        black.position = CGPoint(x: xPos, y: yPos)
        black.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: xWidth, height:5))
        //go through the posArr to see if the next one has similar y value -> keep adding until it isn't
        black.physicsBody?.affectedByGravity = false
        black.physicsBody?.isDynamic = false
        
        black.physicsBody?.categoryBitMask = CollisionBitMask.blackCategory
        black.physicsBody?.collisionBitMask = CollisionBitMask.blackCategory
        black.physicsBody?.contactTestBitMask = CollisionBitMask.birdCategory
        
//        print(xPos, " ", yPos, " ", xWidth)
...
//Automatically spawns walls, flowers
